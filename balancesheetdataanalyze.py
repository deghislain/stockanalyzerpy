...
def get_column_by_parameter(param):
    column_name = ""
    if param == "equity":
        column_name = "total_shareholder_equity"
    if param == "assets":
        column_name = "total_assets"
    if param == "cash":
        column_name = "cash_short_term_investments"
    if param == "inventory":
...
def get_stock_param_by_time_period(PG_URL, period_start, period_end, symbol, param):
    ...
    sql = "SELECT " + column + ", stock_fiscale_date FROM balance_sheet_stock_data WHERE stock_fiscale_date BETWEEN \'" + period_start + "\' AND \'" + period_end + "\' AND stock_symbol = \'" + symbol + "\' order by stock_fiscale_date"
...
def draw_the_curve(period_start, period_end, symbols, param):
    ...
    for s in symbols:
        ...
        if param == "equity":
            y = get_str_stock_data_array(data.total_shareholder_equity)
        if param == "assets":
            y = get_str_stock_data_array(data.total_assets)
        if param == "cash":
            y = get_str_stock_data_array(data.cash_short_term_investments)
        if param == "inventory":
            y = get_str_stock_data_array(data.inventory)
        if param == "investments":
            y = get_str_stock_data_array(data.investments)
        if param == "liabilities":
            y = get_str_stock_data_array(data.total_liabilities)
        if param == "debt":
            y = get_str_stock_data_array(data.current_debt)
        if param == "linvestments":
            y = get_str_stock_data_array(data.long_term_investments)
        plt.plot(x, y)
